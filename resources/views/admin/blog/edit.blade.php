...
@endsection
@section('content')
    <form action="{{route($model.'.update', $blog->id)}}" method="post" enctype="multipart/form-data" class="mt-4">
        @csrf
        @method('put')
        ...
        <div class="position-relative row form-group">
            ...
            <div class="col-md-9 col-xl-8">
                <img style="height: 200px; cursor: pointer;" class="thumbnail" data-toggle="tooltip"
                     title="" data-placement="bottom" src="{{asset($blog->path_image)}}"
        ...
        <div class="position-relative row form-group">
            ...
            <input required="" name="status" id="display" placeholder="Name" type="radio" value="1" {{$blog->status == 1 ? 'checked' : ''}}>
            <label for="hidden" class="mx-3 col-form-label">Ẩn</label>
            <input required="" name="status" id="hidden" placeholder="Name" type="radio" value="0" {{$blog->status == 0 ? 'checked' : ''}}>
        </div>
